...
from gui_components.ai_controls_section_widget import ControlsSectionWidget
...
class AIFeaturesWindow(QMainWindow):
    ...
    def generate_func(self):
        ...
        func_constraint = read_attr_from_settings(f'{self.function_name}_attr')

        in_text = self.in_txtArea.toPlainText()

        if in_text.strip() != "":

            if self.function_name in ['Check Grammar/Spelling',]:
                make_thread_for_ai_task(
                    self,
                    func=lambda: all_tasks_functions[self.function_name](
                        in_text,
                    )
                )
                return
